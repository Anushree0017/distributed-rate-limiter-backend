...
└── requirements.txt
```
Slot new code into these existing folders per the file plan below. Create subfolders inside them
where noted (e.g. `core/rate_limiter/`) rather than new top-level folders.
...
### 1. `RateLimiter` interface + implementations — `core/rate_limiter/`
```
core/rate_limiter/
├── __init__.py
...
### 2. `RateLimiterFactory` — `core/rate_limiter/factory.py`
...
- Add any new dependencies (e.g. `PyYAML`) to `requirements.txt`.
- Keep algorithm implementations free of FastAPI/HTTP concerns — they should be plain Python usable
